...
export function AdminLayout() {
  ...
  // The player dashboard is a separate deployment now -- a USER account should never reach
  // here at all (the backend rejects non-admin logins from this app's origin once
  // ADMIN_ORIGIN is configured), but as defense in depth for local dev where that's unset,
  // log out rather than navigating to "/", which is this same admin-only route.
  if (user.role !== "ADMIN") {
